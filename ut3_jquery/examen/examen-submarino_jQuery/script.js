...
function actividad1Examen(){
  let barcos = $j("#sonar").find(".barco")
  $j("#tab-armas").append("<br>")
  $j("#tab-armas").append('<select id="seleccionar-objetivo"></select>')
  $j("#seleccionar-objetivo").append(`<option value="">----Seleccione Barco----</option>`)
  for(let i = 0; i<barcos.length; i++){
    //console.log($j(barcos[i]).data("id"));
    $j("#seleccionar-objetivo").append(`<option value="${$j(barcos[i]).data("id")}">${$j(barcos[i]).data("id")}</option>`)
  }

  // $j("#seleccionar-objetivo").selectmenu();
  $j("#seleccionar-objetivo").selectmenu({
    change: function( event, ui ) {
      $j(".barco").removeClass("barco-seleccionado")
      let nombreBarcoSeleccionado = $j(this).find(":selected").text()
      console.log(nombreBarcoSeleccionado)
      let barcoBuscado;
      $j("#sonar").find(".barco").each(function(){
        if($j(this).data("id") === nombreBarcoSeleccionado){
          barcoBuscado = this;
        }
      });
      $j(barcoBuscado).addClass("barco-seleccionado")
    }
  });
}
